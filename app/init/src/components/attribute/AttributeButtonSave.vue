...
export default {
  ...
  methods: {
    saveAttribute() {
      ...
      // Clean inputs
      if (this.attribute.order) {
        this.attribute.order = parseInt(this.attribute.order);
      }
      if (this.attribute.linkedAttributeId) {
        this.attribute.dataTypeId = 5;
          ...
          .then(
            function(response) {
              if (response.data.errors) {
                ...
                this.attribute.updatedDate =
                  response.data.data.updateSysAttributeById.sysAttribute.updatedDate;
                this.attribute.sysUserByUpdatedById.email =
                  response.data.data.updateSysAttributeById.sysAttribute.sysUserByUpdatedById.email;
              }
            },
            // Error callback
            function(response) {
          ...
          .then(
            function(response) {
              if (response.data.errors) {
                ...
                this.attribute.id =
                  response.data.data.createSysAttribute.sysAttribute.id;
                this.$router.push({
